...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "frontend" because\n';
  ...
  $wnd.__gwt_activeModules['frontend'] = {
    'moduleName' : 'frontend',
    'bindings' : function() {
      return {};
    }
 ...
 * "/frontend.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['frontend'].superdevmode = true;

  var expectedSuffix = '/frontend.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + frontend + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:frontend';
  var devModeSessionKey = '__gwtDevModeSession:frontend';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/frontend/frontend.nocache.js';
  ...
  $wnd.__gwt_activeModules['frontend'].canRedirect = true;
  $wnd.__gwt_activeModules['frontend'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/frontend?user.agent=' + ua + '&_callback=' + callback;
